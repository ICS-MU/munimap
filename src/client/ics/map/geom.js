 ...
 * @const
 */
ics.map.geom.CENTER_GEOMETRY_FUNCTION = function(feature) {
  var center;
  goog.asserts.assertInstanceof(feature, ol.Feature);
  var geom = feature.getGeometry();
  goog.asserts.assertInstanceof(geom, ol.geom.Geometry);
  if (geom instanceof ol.geom.MultiPolygon) {
    geom = ics.map.geom.getLargestPolygon(geom);
  }
  center = ics.map.geom.getGeometryCenter(
      geom, ics.map.building.isBuilding(feature));
  return center;
};
 ...
 * @const
 */
ics.map.geom.INTERSECT_CENTER_GEOMETRY_FUNCTION = function(map, feature) {
  var center;
  ...
  var featCenter = ics.map.geom.CENTER_GEOMETRY_FUNCTION(feature);
  if (geom.intersectsExtent(refExt) && !featCenter.intersectsExtent(refExt)) {
    var format = new ol.format.GeoJSON();
    var intersect =
        ics.map.geom.featureExtentIntersect(feature, viewExt, format);
 ...
 * @protected
 */
ics.map.geom.getGeometryCenter = function(geometry, opt_useOlInteriorPoint) {
  var center;
  if (geometry instanceof ol.geom.Polygon) {
    if (opt_useOlInteriorPoint) {
      center = geometry.getInteriorPoint();
    } else {
      center = ics.map.geom.getBetterInteriorPoint(geometry);
    }
  } else {
 ...
 * @return {ol.Feature}
 */
ics.map.geom.featureExtentIntersect = function(feature, extent, format) {
  var turfFeature =
 ...
 * @protected
 */
ics.map.geom.getLinearRingHorizontalIntersections = function(ring, y) {
 ...
 * @protected
 */
ics.map.geom.getHorizontalIntersections = function(polygon, y) {
  var intersections = [];
  goog.array.forEach(polygon.getLinearRings(), function(ring) {
    var ringIntersections =
        ics.map.geom.getLinearRingHorizontalIntersections(ring, y);
 ...
 * @protected
 */
ics.map.geom.getBetterInteriorPoint = function(polygon) {
  ...
  if (polygon.containsCoordinate(centerCoordinate)) {
    ...
    var intersections =
        ics.map.geom.getHorizontalIntersections(polygon, resultY);
    intersections.sort();
 ...
 * @protected
 */
ics.map.geom.getLargestPolygon = function(multipolygon) {
